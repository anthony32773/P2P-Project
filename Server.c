...
int main(int argc, char **argv)
{
	...
	printf("P2P Network Started and Initialized\nWaiting for Client Requests...\n");
	...
	while (1)
	{
		...
		else if (recvData.type == 'S')
		{
			...
			if (foundContent == true)
			{
				...
				int k;
				response.type = 'S';
				aLen = strlen(peerList[i].address);
				aLen++;
				pLen = strlen(peerList[i].port);
				pLen++;
				memcpy(response.data, peerList[i].address, aLen);
				memcpy(response.data + aLen, peerList[i].port, pLen);
				sendto(server_socket, (struct udpPDU *) &response, sizeof(response), 0, (struct sockaddr*) &client_address, addrSize);

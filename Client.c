...
void printOptions()
{
	printf("Enter R to register content with the network\n");
	printf("Enter S to download a content from the network\n");
	printf("Enter O to List all content available on the network\n");
	printf("Enter T to De-Register a certain piece of content on the network\n");
	printf("Enter Q to De-Register all content from the network and be removed from the Peer list\n");
}
...
void registration(struct udpPDU* sendData, struct udpPDU* response, bool * registrationStatus, bool * finishSocket, char peerName[], char address[], char contentList[][CONTENTLENGTH], char port[], int * numContent, ssize_t * recvLength, struct sockaddr_in* server_address, socklen_t * addrSize, int network_socket)
{
	...
	if (*registrationStatus == false)
	{
		//Client Reg inputs
		bool portExit = false;
		printf("Enter Desired Peer Name:\n");
		printf("Peer Name Requirements:\n");
		printf("No spaces in name\n");
		printf("Must be less than 20 characters\n");
		scanf("%s", peerName);
		...
		printf("Enter The Name of The Content You Would Like to Host:\n");
		printf("Content name requirements:\n");
		printf("No spaces in the file name\n");
		printf("Must be less than 20 characters\n");
		scanf("%s", contentTemp);
		if (strlen(contentTemp) >= 20)
		{
		...
		{
			printf("Enter port number to host the content on:\n");
			scanf("%s", port);
			portExit = true;
